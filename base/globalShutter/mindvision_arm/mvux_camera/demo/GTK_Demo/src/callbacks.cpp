...
gboolean   on_res_combobox_changed      (GtkWidget *widget,gpointer     user_data)
{
    ...
    int                       iImageSizeDesc=g_tCapability.iImageSizeDesc; // 预设分辨率的个数，即pImageSizeDesc数组的大小
...
void on_d_button_confirm_pressed         (GtkButton       *button, gpointer         user_data)
{
    GtkWidget *delay_entry   =lookup_widget(main_window, "delay_entry");
    UINT   uDelayTimeUs=atoi(gtk_entry_get_text( (GtkEntry  *)delay_entry ));;
    CameraSetStrobeDelayTime(g_hCamera,uDelayTimeUs);
    return ;
}


void on_p_button_confirm_pressed         (GtkButton       *button, gpointer         user_data)
{
    GtkWidget *pulse_entry   =lookup_widget(main_window, "pulse_entry");
    UINT   uTimeUs=atoi(gtk_entry_get_text( (GtkEntry  *)pulse_entry ));
    CameraSetStrobePulseWidth(g_hCamera,uTimeUs);
    return ;
}
...
void on_radio_flashlight                      (GtkWidget       *radio,int         user_data)
{
    ...
    switch(user_data){
        ...
        case 1:
            ...
            CameraGetStrobePulseWidth(g_hCamera,&upTimeUs);
            CameraGetStrobeDelayTime(g_hCamera,&upDelayTimeUs);

            memset(buffer,0,sizeof(buffer));
            sprintf(buffer,"%d",upDelayTimeUs);
            gtk_entry_set_text((GtkEntry      *)delay_entry, buffer);

            memset(buffer,0,sizeof(buffer));
            sprintf(buffer,"%d",upTimeUs);
            gtk_entry_set_text((GtkEntry      *)pulse_entry, buffer);
        break;
...
void on_exposure_time_value_changed (GtkScale *scale, gpointer data)
{
    ...
    GtkWidget *entry_exp_time=lookup_widget(main_window, "entry_exp_time");
...
*/
    ...
    gtk_entry_set_text((GtkEntry      *)entry_exp_time, buffer);
    return ;

}

void on_entry_exp_time (GtkWidget *widget,GdkEventKey *event)
{
    ...
    if(strlen(gtk_entry_get_text((GtkEntry      *)widget))>15){
        return ;
    }

    tmp=atof(gtk_entry_get_text( (GtkEntry  *)entry_exp_time ));
...
/*
    ...
    来获得实际设定的值。
*/
    iPos=tmp*1000/m_fExpLineTime;

    CameraSetExposureTime(g_hCamera,iPos*m_fExpLineTime);

    gtk_range_set_value((GtkRange      *)scale_exposure_time,iPos);
